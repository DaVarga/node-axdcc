...
class Axdcc extends EventEmitter {
    /**
     * Request constructor.
     * @param {IRC} client - Irc client from Node Irc library
    ...
    constructor(client, opts) {
        ...
        if(!_.isString(opts.nick) || !_.isString(opts.path))
            throw new Error("opts.nick and opts.path must be a string");
    ...
    download() {
        ...
        stream.on("open", () => {
            ...
            conn.on("data", data => {
                ...
                pack.received += data.length;
                sendBuffer.writeUInt32BE(pack.received & 0xFFFFFFFF, 0);
            ...
            conn.on("end", () => {
                ...
                stream.end();
                // Connection closed
                if (pack.received == pack.fileSize) {// Download complete
                    this.complete();
    ...
    /**
     * gets the irc client
     * @returns {IRC} irc client reference
     * @public
     */
    get client() {
